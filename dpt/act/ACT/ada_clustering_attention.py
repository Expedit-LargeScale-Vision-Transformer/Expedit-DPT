from .extensions import ada_cluster_, broadcast_, weighted_sum_
...
class CalcCenter(torch.autograd.Function):
    ...
    def forward(ctx, x, clusters, counts):
        ...
        center = weighted_sum_(x, clusters, weights)
    ...
    def backward(ctx, grad_center):
        ...
        grad = broadcast_(grad_center, clusters, weights)
...
class Broadcast(torch.autograd.Function):
    ...
    def forward(ctx, center, clusters):
        ...
        x = broadcast_(center, clusters, weights)
    ...
    def backward(ctx, grad):
        ...
        grad_center = weighted_sum_(grad, clusters, weights)
...
class AdaClusteringAttention(Module):
    ...
    def _create_clusters(self, V, n_hashes=32):
        B, N, D = V.shape
        hashes = self._compute_hashes(V, n_hashes=n_hashes)
        groups, counts = ada_cluster_(hashes)
